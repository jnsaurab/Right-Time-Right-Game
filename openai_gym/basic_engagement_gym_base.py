...
class BasicEngagementGymBase(gym.Env):
    ...
    def reset(self):
        # set chronometer which automatically skips 11pm to 8am because it's usually when people
        # sleep
        self.chronometer = Chronometer(
                refDay=self.masterNumDayPassed % 7,
                skipFunc=(lambda hour, _m, _d: hour < 8 or hour >= 23),
        )
    ...
    def _generate_reward(self, action):
        ...
        probReactions = self.environment.getResponseDistribution(
                currentHour, currentMinute, currentDay, self.lastNotificationTime,
        )
        probReactions = utils.normalize(*probReactions)
        probAnsweringNotification, probIgnoringNotification = probReactions
